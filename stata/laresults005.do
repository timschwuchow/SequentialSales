...
reg `regvardif' c.sellpctdif##c.(`heterolist') `heterolistforward' `avgextdiflist' timedum1-timedum`ntimedum' if notdif2==1 & newsale==1, r cluster(bid)
...
xtreg `regvardif' c.sellpctdif##c.(`heterolist') `heterolistforward' `avgextdiflist' timedum1-timedum`ntimedum' if notdif2==1 & newsale, r fe 
...
xtset ddcell
xtreg lpdif agedif ltselldif c.sellpctdif##c.(`heterolist') `extdiflist' if ddinc==1 & newsale==1 & isincluded == 1, r fe
...
// bysort bid (sellorder ab): gen nearadj = (overlinc==overlinc[_n-1])
// bysort bid (sellorder ab): gen lpdifadj = lp - lp[_n-1]
// bysort bid (sellorder ab): gen sqftdifadj = sqft - sqft[_n-1]
// bysort bid (sellorder ab): gen numbeddifadj = numbed - numbed[_n-1]
// bysort bid (sellorder ab): gen numbathdifadj = numbath - numbath[_n-1]
// bysort bid (sellorder ab): gen ltselldifadj = ltsell - ltsell[_n-1]
