...
// miasumstatxxx.do - computes some summary statistics for developer-built units 


local filename "miasumstat${version}"
...
graph export ${logdir}miabsizedensity.ps, replace
!/usr/bin/mogrify -trim -density 200 -format jpg ${logdir}miabsizedensity.ps 
!rm ${logdir}miabsizedensity.ps
...
format `vars' %5.3f
sutex `vars' if newsale == 1, digits(5) labels file(${logdir}miasumstatnew${version}.tex) replace nobs minmax 
sutex `vars' if newsale == 0, digits(5) labels file(${logdir}miasumstatold${version}.tex) replace nobs minmax 
foreach x in `vars' {
	sum `x' if newsale == 1 
}
...
sutex cumav* av*end meanext*, digits(5) labels file(${logdir}miasumstatext${version}.tex) replace nobs minmax 
