...
// clesumstatxxx.do - computes some summary statistics for developer-built units 


local filename "clesumstat${version}"
log using ${logdir}`filename'.txt, replace text name(`filename') 
use ${datdir}clefinal${version}.dta, clear 
...
graph export ${logdir}clebsizedensity.ps, replace
!/usr/bin/mogrify -trim -density 200 -format jpg ${logdir}clebsizedensity.ps 
!rm ${logdir}clebsizedensity.ps
...
format `vars' %5.3f
sutex `vars' if newsale == 1, digits(5) labels file(${logdir}clesumstatnew${version}.tex) replace nobs minmax 
sutex `vars' if newsale == 0, digits(5) labels file(${logdir}clesumstatold${version}.tex) replace nobs minmax 
foreach x in `vars' {
	sum `x' if newsale == 1 
}
...
sutex cumav* av*end meanext*, digits(5) labels file(${logdir}clesumstatext${version}.tex) replace nobs minmax 
